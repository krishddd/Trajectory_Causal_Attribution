...
class ReplayContext(AgentContext):
    ...
    def _op(
        ...
        if rec_idx is None:
            # No recorded counterpart for this operation: the timeline diverged
            # (an upstream ablation changed the control flow). Only live
            # resampling is meaningful here.
            self.diverged = True
        ...
        if decision == "force":
            return self.plan.forced[rec_idx]
        if decision == "remove":
            return REMOVED
        if decision == "hold":
            return step.output
        ...
        if not step.resamplable or produce is None:
            return step.output
        return produce()
